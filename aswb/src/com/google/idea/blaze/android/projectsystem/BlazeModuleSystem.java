...
public class BlazeModuleSystem implements AndroidModuleSystem, BlazeClassFileFinder {
  private static final Logger LOG = Logger.getInstance(BlazeModuleSystem.class);
  ...
  private ExternalLibrary toExternalLibrary(
    ...
    if (cachedJar == null) {
      LOG.warn(
          String.format(
              "Failed to locate jar file %s. Re-sync project may solve the problem",
              library.toString()));
      return null;
    }
